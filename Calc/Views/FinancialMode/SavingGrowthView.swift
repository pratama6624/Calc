...
struct SavingGrowthView: View {
    @StateObject private var viewModel = SavingGrowthViewModel()
            ...
            VStack(spacing: 25) {
                ...
                VStack(spacing: 8) {
                    VStack(alignment: .trailing, spacing: 6) {
                        Text("Initial: \(viewModel.initialSaving)")
                        Text("Rate: \(viewModel.interestRate)%")
                        Text("Duration: \(viewModel.durationYears) yrs")
                    }
                    ...
                    ZStack {
                        ...
                        } else if !viewModel.totalAmount.isEmpty {
                            VStack(spacing: 6) {
                                ...
                                Text("Interest: IDR \(viewModel.totalInterest)")
                                    .font(.caption)
                                    .foregroundColor(.orange)
                                    .frame(maxWidth: .infinity, alignment: .trailing)
                                    .padding(.horizontal, 14)
                ...
                HStack(spacing: 14) {
                    fieldButton("Initial", active: .initialSaving)
                    fieldButton("Rate", active: .interestRate)
                    fieldButton("Years", active: .durationYears)
                }
                .padding(.horizontal, 14)
                ...
                HStack(spacing: 12) {
                    ...
                    Button("CALCULATE") {
                        viewModel.calculateSavingGrowth()
                    }
                    .font(.title3)
                    .fontWeight(.bold)
    ...
    // MARK: - Field Button
    private func fieldButton(_ title: String, active field: SavingGrowthViewModel.ActiveField) -> some View {
        Button(action: {
